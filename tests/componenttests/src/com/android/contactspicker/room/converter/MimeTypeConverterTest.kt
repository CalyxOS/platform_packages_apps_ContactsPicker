...
import com.android.contactspicker.ui.utils.SUPPORTED_MIME_TYPES
...
class MimeTypeConverterTest {
    ...
    fun fromMimeTypeList_eachSupportedType_mapsToUniqueBitmask() {
        ...
        assertThat(uniqueBitmasks.size).isEqualTo(SUPPORTED_MIME_TYPES.size)
    }
    ...
    fun fromMimeTypeList_multipleValidTypes_returnsCorrectBitmask() {
        val mimeTypes =
            listOf(
                ContactsContract.CommonDataKinds.Email.CONTENT_ITEM_TYPE, // bit 2
                ContactsContract.CommonDataKinds.Photo.CONTENT_ITEM_TYPE, // bit 7
            )
    ...
    @Test
    fun fromMimeTypeList_unsupportedType_isIgnored() {
        val mimeTypes = listOf("unsupported/mime.type")
        val bitmask = mimeTypeConverter.fromMimeTypeList(mimeTypes)
    ...
    fun toMimeTypeList_eachSingleBit_mapsToUniqueType() {
        val allConvertedMimeTypes = mutableListOf<String>()
        SUPPORTED_MIME_TYPES.forEachIndexed { index, _ ->
        ...
        assertThat(allConvertedMimeTypes).containsExactlyElementsIn(SUPPORTED_MIME_TYPES)
    }
    ...
    fun conversion_isReversible() {
        val bitmask = mimeTypeConverter.fromMimeTypeList(SUPPORTED_MIME_TYPES)
        val convertedMimeTypes = mimeTypeConverter.toMimeTypeList(bitmask)

        assertThat(convertedMimeTypes).containsExactlyElementsIn(SUPPORTED_MIME_TYPES)
    }
}
